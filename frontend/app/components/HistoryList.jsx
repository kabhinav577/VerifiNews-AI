               ...
               <div className="flex gap-4 mt-4 pt-4 border-t border-slate-100 dark:border-brand-border">
                  <div className="flex items-center gap-2">
                     ...
                     <span className={`text-xs font-semibold px-2 py-0.5 rounded ${
                        item.prediction.toLowerCase().includes('fake') ? 'bg-orange-50 text-orange-700' : 'bg-green-50 text-green-700'
                     }`}>
                        {item.prediction.toLowerCase().includes('fake') ? 'High' : 'Low'}
                     </span>
                  </div>
                  <div className="flex items-center gap-2">
                     <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Sentiment:</span>
                     <span className={`text-xs font-semibold px-2 py-0.5 rounded ${
                        item.prediction.toLowerCase().includes('fake') ? 'bg-red-50 text-red-700' : 'bg-blue-50 text-blue-700'
                     }`}>
                        {item.prediction.toLowerCase().includes('fake') ? 'Negative' : 'Neutral'}
                     </span>
                  </div>
               </div>
